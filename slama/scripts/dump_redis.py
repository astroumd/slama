            ...
            for kk,vv in dict(sorted(v.items())).items():
               ...
               if type(vv) == dict:
                    for kkk,vvv in dict(sorted(v.items())).items():
                       ...
                       if top.strip() != "":
                           if args.values:
                               print(f'{top}:{middle}:{bottom}={vvv.decode()}')
                           else:
                               print(f'{top}:{middle}:{bottom}')
                       else:
                           if args.values:
                               print(f'{middle}:{bottom}={vvv.decode()}')
                           else:
                               print(f'{middle}:{bottom}')
               else:
                   if top.strip() != "":
                       if args.values:
                           print(f'{top}:{middle}={vv.decode()}')
                       else:
                           print(f'{top}:{middle}')
                   else:
                       if args.values:
                           print(f'{middle}={vv.decode()}')
                       else:
                           print(f'{middle}')
        else:
            if args.values:
                print(f'{top}={v.decode()}')
            else:
                print(top)
else:
    for k,v in dict(sorted(dump.items())).items():
